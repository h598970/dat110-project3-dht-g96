...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		if(areAllMessagesReturned(peer.size())) {
			// if yes, acquireLock
			acquireLock();
	...
	public void onMutexRequestReceived(Message message) throws RemoteException {
		...
		if(message.getNodeID().equals(node.getNodeID())) {
			...
			// call onMutexAcknowledgementReceived()
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException {
		...
		switch(condition) {
			...
			case 2: {
				...
				if(senderClock < ownClock || (senderClock == ownClock && message.getNodeID().compareTo(node.getMessage().getNodeID()) < 0)) {
	...
	public void multicastReleaseLocks(Set<Message> activenodes) {
		...
		for(Message m : activenodes) {
			...
			NodeInterface stub;
			try {
				stub = Util.getProcessStub(m.getNodeName(), m.getPort());
				// call releaseLocks()
				stub.releaseLocks();
